...
public class PCIeRootPort extends BasicComponent {
	...
	private void updateTxsNode(DTNode node) {
		for(int i=0; i<node.getPropertyByName("reg-names").getValues().size(); i++) {
			if(node.getPropertyByName("reg-names").getValues().get(i).toString().equals("\"txs\""))
				node.getPropertyByName("reg-names").getValues().set(i, new DTPropStringVal("Txs"));
			if(node.getPropertyByName("reg-names").getValues().get(i).toString().equals("\"cra\""))
				node.getPropertyByName("reg-names").getValues().set(i, new DTPropStringVal("Cra"));
		}
	}
